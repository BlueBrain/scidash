...
from .converters import ttest_calc, zscore3
...
def run_cclamp(path, force_run, model, amp):
	"""Runs a current clamp on the model and return membrane voltage trace. Uses default 
	location (usually the soma) for placing the current clamp.

	Parameters
	==========
	path : string
		   Directory where clamped traces are saved and loaded.
	force_run : Bool
		        If true then always runs the current clamp. If false runs the current clamp
		        only if an existing traces doesn't already exists in path.
	model : sciunit.Model
	        Model with CurrentClamp capabilities.
	amp : float
	      Value of current in milliamps.

	Returns
	=======
	traces : list
	         Information of the trace of the current clamp; time, membrane voltage, stimulation begin,
	         stimulation end, number of spikes.
	"""
...
class DepolarizationBlockTest(Test):
	...
	def generate_prediction(self, model):		
		...
		amps = np.arange(0, 1.65, 0.05) # 0 to 1.65 Amps, in steps of .05 A
		...
		# Determines the minimum current (Ith) needed to produce a maximum number of action-potentials.
	...
	def find_Ith_Veq(self, model, results, amps):
		...
		spikecount_array = np.array([])
		for i, amp in enumerate(amps):
			spikecount_array = np.append(spikecount_array, results[i][1][0]['Spikecount'])

		max = np.amax(spikecount_array)
		Ith_index = np.where(spikecount_array==max)[0]
		if type(Ith_index) is np.ndarray: # If Ith index has multiple values...  
		...
		if Ith_index == (spikecount_array.size-1):
			...
			time = np.array(results[Veq_index][0]['T'])
			indices1 = np.where(1400<=time)[0]           # we need the last 100ms of the current pulse
			indices2 = np.where(1500>=time)[0]
			trace_end_index_beginning = np.amin(indices1)
			trace_end_index_end = np.amax(indices2)
			trace_end = Veq_trace[trace_end_index_beginning:trace_end_index_end]      # THIS CONTAINS the last 100ms of the current pulse
			Veq = np.average(trace_end)

		# Save the plots 
		if self.show_plot:
			path_figs = os.path.join(self.fig_directory, model.name + '/depol_block/')
...
def analyse_syn_traces(t, v, v_dend, threshold):
	"""Returns trace information, spike count, of the membrane potential at the soma (v) and the
	synapse (v_dend).
...
def binsearch(model, path_bin_search, force_run, dend_loc0):
	...
	if force_run or (os.path.isfile(file_name) is False):
		...
		# Set up model to stimulate the synapse and record potential at synapse and soma
		model.initialise()
		...
		while c_step_start >= c_step_stop and not found:
			c_stim = np.arange(c_minmax[0], c_minmax[1], c_step_start)

			first = 0
			last = np.size(c_stim, axis=0) - 1
...
def run_synapse(model, path, force_run, dend_loc_num_weight, interval):
	...
	if force_run or (os.path.isfile(file_name) is False):
		...
		# Induce a network stimulation at the synapse, record potential at the soma (v) and synapse (v_dend)
		model.initialise()
...
class ObliqueIntegrationTest(Test):
	...
	def calcs_plots(self, model, results, dend_loc000, dend_loc_num_weight, path_figs):
		...
		sep=np.arange(0, stop, 11)
		sep_results=[]

		max_num_syn=10
		num = np.arange(0,max_num_syn+1)
		...
		dV_dt=[]
		sep_dV_dt=[]
		soma_max_depols=np.array([])
		soma_expected=np.array([])
		sep_soma_max_depols=[]
		sep_soma_expected=[]
		max_dV_dt=np.array([])
		sep_max_dV_dt=[]
		max_dV_dt_index=np.array([],dtype=np.int64)
		sep_threshold=np.array([])
		prox_thresholds=np.array([])
		dist_thresholds=np.array([])
		peak_dV_dt_at_threshold=np.array([])
		nonlin=np.array([])
		suprath_nonlin=np.array([])
		amp_at_threshold=[]
		sep_time_to_peak=[]
		time_to_peak_at_threshold=np.array([])
		time_to_peak=np.array([])
		...
		# calculating somatic depolarization and first derivative
				...
				soma_max_depols=np.append(soma_max_depols, np.amax(soma_depol))

				dt=np.diff(sep_results[i][j][0][0]['T'] )
				dV=np.diff(sep_results[i][j][0][0]['V'] )
				deriv=dV/dt
				dV_dt.append(deriv)

				max_dV_dt=np.append(max_dV_dt, np.amax(dV_dt))

				diff_max_dV_dt=np.diff(max_dV_dt)

				if j==0:
					soma_expected=np.append(soma_expected,0)
				else:
					soma_expected=np.append(soma_expected,soma_max_depols[1]*j)

				if j!=0:
					peak=np.amax(soma_depol)

					peak_index=np.where(soma_depol==peak)[0]
					peak_time=sep_results[i][j][0][0]['T'][peak_index]
					t_to_peak=peak_time-150
					time_to_peak = np.append(time_to_peak, t_to_peak)
				else:
					time_to_peak = np.append(time_to_peak, 0)

				#print time_to_peak

			threshold_index0=np.where(diff_max_dV_dt==np.amax(diff_max_dV_dt[1:]))[0]
			threshold_index0=np.add(threshold_index0,1)
			...
			if sep_results[i][threshold_index0][3] > 1 and sep_results[i][threshold_index0-1][3]==1:    #double spikes can cause bigger jump in dV?dt than the first single spike, to find the threshol, we want to eliminate this, but we also need the previous input level to generate spike
				threshold_index=np.where(diff_max_dV_dt==np.amax(diff_max_dV_dt[1:threshold_index0-1]))
				threshold_index=np.add(threshold_index,1)
			else:
			...
			sep_threshold=np.append(sep_threshold, threshold)
			peak_dV_dt_at_threshold=np.append(peak_dV_dt_at_threshold,max_dV_dt[threshold_index])
			nonlin=np.append(nonlin, soma_max_depols[threshold_index]/ soma_expected[threshold_index]*100)  #degree of nonlinearity
			suprath_nonlin=np.append(suprath_nonlin, soma_max_depols[threshold_index+1]/ soma_expected[threshold_index+1]*100)  #degree of nonlinearity
			amp_at_threshold=np.append(amp_at_threshold, soma_max_depols[threshold_index])
			sep_time_to_peak.append(time_to_peak)
			time_to_peak_at_threshold=np.append(time_to_peak_at_threshold, time_to_peak[threshold_index])
		...
		threshold_errors = np.array([abs(experimental_mean_threshold - threshold_errors*mV)/threshold_SD  for threshold_errors in sep_threshold])     # does the same calculation on every element of a list  #x = [1,3,4,5,6,7,8] t = [ t**2 for t in x ]
		prox_threshold_errors=np.array([abs(threshold_prox - prox_threshold_errors*mV)/threshold_prox_SD  for prox_threshold_errors in prox_thresholds])        # and I could easily make it a np array : t = np.array([ t**2 for t in x ])
		dist_threshold_errors=np.array([abs(threshold_dist - dist_threshold_errors*mV)/threshold_dist_SD  for dist_threshold_errors in dist_thresholds])
		peak_deriv_errors=np.array([abs(exp_mean_peak_deriv - peak_deriv_errors*mV /ms )/deriv_SD  for peak_deriv_errors in peak_dV_dt_at_threshold])
		nonlin_errors=np.array([abs(exp_mean_nonlin- nonlin_errors)/nonlin_SD  for nonlin_errors in nonlin])
		suprath_nonlin_errors=np.array([abs(suprath_exp_mean_nonlin- suprath_nonlin_errors)/suprath_nonlin_SD  for suprath_nonlin_errors in suprath_nonlin])
		amplitude_errors=np.array([abs(exp_mean_amp- amplitude_errors*mV)/amp_SD  for amplitude_errors in amp_at_threshold])
		time_to_peak_errors=np.array([abs(exp_mean_time_to_peak- time_to_peak_errors*ms)/exp_mean_time_to_peak_SD  for time_to_peak_errors in time_to_peak_at_threshold])


		# means and SDs
		mean_threshold_errors=np.mean(threshold_errors)
		mean_prox_threshold_errors=np.mean(prox_threshold_errors)
		mean_dist_threshold_errors=np.mean(dist_threshold_errors)
		mean_peak_deriv_errors=np.mean(peak_deriv_errors)
		mean_nonlin_errors=np.mean(nonlin_errors)
		suprath_mean_nonlin_errors=np.mean(suprath_nonlin_errors)
		mean_amplitude_errors=np.mean(amplitude_errors)
		mean_time_to_peak_errors=np.mean(time_to_peak_errors)

		sd_threshold_errors=np.std(threshold_errors)
		sd_prox_threshold_errors=np.std(prox_threshold_errors)
		sd_dist_threshold_errors=np.std(dist_threshold_errors)
		sd_peak_deriv_errors=np.std(peak_deriv_errors)
		sd_nonlin_errors=np.std(nonlin_errors)
		suprath_sd_nonlin_errors=np.std(suprath_nonlin_errors)
		sd_amplitude_errors=np.std(amplitude_errors)
		sd_time_to_peak_errors=np.std(time_to_peak_errors)


		mean_sep_threshold=float(np.mean(sep_threshold)) *mV
		mean_prox_thresholds=float(np.mean(prox_thresholds)) *mV
		mean_dist_thresholds=np.mean(dist_thresholds) *mV
		mean_peak_dV_dt_at_threshold=np.mean(peak_dV_dt_at_threshold) *V /s
		mean_nonlin=np.mean(nonlin)
		suprath_mean_nonlin=np.mean(suprath_nonlin)
		mean_amp_at_threshold=np.mean(amp_at_threshold) *mV
		mean_time_to_peak_at_threshold=np.mean(time_to_peak_at_threshold) *ms

		sd_sep_threshold=float(np.std(sep_threshold)) *mV
		sd_prox_thresholds=float(np.std(prox_thresholds)) *mV
		sd_dist_thresholds=np.std(dist_thresholds) *mV
		sd_peak_dV_dt_at_threshold=np.std(peak_dV_dt_at_threshold) *V /s
		sd_nonlin=np.std(nonlin)
		suprath_sd_nonlin=np.std(suprath_nonlin)
		sd_amp_at_threshold=np.std(amp_at_threshold) *mV
		sd_time_to_peak_at_threshold=np.std(time_to_peak_at_threshold) *ms


		depol_input=np.array([])
		mean_depol_input=[]
		SD_depol_input=[]
		SEM_depol_input=[]

		expected_depol_input=np.array([])
		expected_mean_depol_input=[]

		prox_depol_input=np.array([])
		prox_mean_depol_input=[]
		prox_SD_depol_input=[]
		prox_SEM_depol_input=[]

		prox_expected_depol_input=np.array([])
		prox_expected_mean_depol_input=[]

		dist_depol_input=np.array([])
		dist_mean_depol_input=[]
		dist_SD_depol_input=[]
		dist_SEM_depol_input=[]

		dist_expected_depol_input=np.array([])
		dist_expected_mean_depol_input=[]

		peak_deriv_input=np.array([])
		...
		for j in range (0, max_num_syn+1):
			for i in range (0, len(sep_soma_max_depols)):
				depol_input=np.append(depol_input,sep_soma_max_depols[i][j])
				expected_depol_input=np.append(expected_depol_input,sep_soma_expected[i][j])
				peak_deriv_input=np.append(peak_deriv_input,sep_max_dV_dt[i][j])
			mean_depol_input.append(np.mean(depol_input))
			expected_mean_depol_input.append(np.mean(expected_depol_input))
			mean_peak_deriv_input.append(np.mean(peak_deriv_input))
			SD_depol_input.append(np.std(depol_input))
			SEM_depol_input.append(np.std(depol_input)/math.sqrt(n))
			SD_peak_deriv_input.append(np.std(peak_deriv_input))
			SEM_peak_deriv_input.append(np.std(peak_deriv_input)/math.sqrt(n))
			depol_input=np.array([])
			expected_depol_input=np.array([])
			peak_deriv_input=np.array([])
		...
		for j in range (0, max_num_syn+1):
			for i in range (0, len(prox_sep_soma_max_depols)):
				prox_depol_input=np.append(prox_depol_input,prox_sep_soma_max_depols[i][j])
				prox_expected_depol_input=np.append(prox_expected_depol_input,prox_sep_soma_expected[i][j])
			prox_mean_depol_input.append(np.mean(prox_depol_input))
			prox_expected_mean_depol_input.append(np.mean(prox_expected_depol_input))
			prox_SD_depol_input.append(np.std(prox_depol_input))
			prox_SEM_depol_input.append(np.std(prox_depol_input)/math.sqrt(prox_n))
			prox_depol_input=np.array([])
			prox_expected_depol_input=np.array([])

		for j in range (0, max_num_syn+1):
			for i in range (0, len(dist_sep_soma_max_depols)):
				dist_depol_input=np.append(dist_depol_input,dist_sep_soma_max_depols[i][j])
				dist_expected_depol_input=np.append(dist_expected_depol_input,dist_sep_soma_expected[i][j])
			dist_mean_depol_input.append(np.mean(dist_depol_input))
			dist_expected_mean_depol_input.append(np.mean(dist_expected_depol_input))
			dist_SD_depol_input.append(np.std(dist_depol_input))
			dist_SEM_depol_input.append(np.std(dist_depol_input)/math.sqrt(dist_n))
			dist_depol_input=np.array([])
			dist_expected_depol_input=np.array([])
	...
	def calcs_plots_async(self, model, results, dend_loc000, dend_loc_num_weight, path_figs):
		...
		for i in range (0, len(sep_results)):
			for j in range (0,max_num_syn+1):
				...
				if j==0:
					soma_expected=np.append(soma_expected,0)
					nonlin=np.append(nonlin,100)
				else:
					soma_expected=np.append(soma_expected,soma_max_depols[1]*j)
					nonlin=np.append(nonlin, soma_max_depols[j]/ soma_expected[j]*100)
			...
			nonlins_at_th=np.append(nonlins_at_th, nonlin[5])      #degree of nonlin at 4 inputs, that is the threshold in the synchronous case
		...
		mean_nonlin_at_th=np.mean(nonlins_at_th)
		SD_nonlin_at_th=np.std(nonlins_at_th)
	...
	def generate_prediction(self, model):
		...
		# Bin search - determine the stimulation weight (if any) needed at each dendritic synapse needed
		# to induce AP at soma		
		traces = []
		...
		path_figs = os.path.join(self.fig_directory, model.name)
		path_figs = os.path.join(path_figs, 'oblique/')
		if self.show_plot:			
			if not os.path.exists(path_figs):
				os.makedirs(path_figs)
			printd("The figures are saved in the directory: ", path_figs)
...
class SomaticFeaturesTest(Test):
	...
	def generate_prediction(self, model):
		...
		if self.show_plot:
			path_figs = os.path.join(self.fig_directory, model.name)
			path_figs = os.path.join(path_figs, 'soma/')
			if not os.path.exists(path_figs):
			...
			putils.plot_absolute_features(path_figs, features_names, feature_results_dict, self.observation)
